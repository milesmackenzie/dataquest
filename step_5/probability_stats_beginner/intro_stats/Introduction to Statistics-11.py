...
degrees_zero = [x + 459.67 for x in fahrenheit_degrees]
...
males = []    
females = []
for idx, item in enumerate(gender):
    if item == "male":
        males.append(savings[idx])
    else:
        females.append(savings[idx])
male_savings = sum(males) / len(males)
female_savings = sum(females) / len(females)
print (male_savings)

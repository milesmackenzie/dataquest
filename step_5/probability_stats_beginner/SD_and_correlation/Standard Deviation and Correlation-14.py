...
median_difference_sum = sum([x - values_median for x in values])
...
variance = 0
for x in nba_stats['pts']:
    diff = x - nba_stats['pts'].mean()
    variance += (diff **2)
...
print (s.stdev(wing_lengths))
print (s.mean(wing_lengths))

std = []
for item in wing_lengths:
    dist = s.mean(wing_lengths) - item
    stds = abs(dist) / s.stdev(wing_lengths)
    std.append(stds)
...
for item in std:
    if item <= 3:
        count3 += 1
within_one_percentage = count/len(wing_lengths)
within_two_percentage = count2/len(wing_lengths)
within_three_percentage = count3/len(wing_lengths)
...
r_fta_pts = pearsonr(nba_stats['fta'], nba_stats['pf'])
r_stl_pf = pearsonr(nba_stats['stl'], nba_stats['pf'])


## 13. Calculating Covariance ##
...
## 14. Calculating Correlation With the std() Method ##

from numpy import cov

covariance2 = (cov(nba_stats["fta"], nba_stats["blk"])[0,1])

stds2 = (nba_stats['fta'].std() * nba_stats['blk'].std())

covariance = (cov(nba_stats["ast"], nba_stats["stl"])[0,1])
stds = nba_stats['ast'].std() * nba_stats['stl'].std()

r_ast_stl = covariance / stds
r_fta_blk = covariance2 / stds2
